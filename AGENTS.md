...
| [08-implementation-tracker.md](docs/08-implementation-tracker.md) | Current implementation status by feature and phase |
...
- Typed APIs, data structures, and contracts are always preferred over untyped or loosely typed alternatives.
- Architecturally clean, readable solutions are always preferred over quick hacks or narrow simple fixes.
- Known, well-documented, widely used solutions are preferred over one-off custom implementations when they fit the problem.
...
- The sidecar protocol (stdin/stdout newline-delimited JSON) is defined by Pydantic models in `analyzer/sonoscope_analyzer/protocol.py`. Change the protocol there first, then update the Rust consumer.

### Generated files
- `src/lib/bindings/` is auto-generated — do not hand-edit.
- `analyzer/mappings/*.json` are hand-maintained configuration files — do not generate them.
...
- Update `08-implementation-tracker.md` whenever implementing, deferring, or materially changing a feature.
